...
public class BankTest {
    ...
    @Test
    void theCheckboxIsNotMarked() { // не отмечен чекбокс
        String planingDate = generateDate(3, "dd.MM.yyyy");
        Selenide.open("http://localhost:9999/");
        ...
        $("[data-test-id='phone'] input").setValue("+79999999999");
        $$("button").findBy(text("Забронировать")).click();
        $$("[data-test-id='agreement'].input_invalid .checkbox__text").findBy(text("Я соглашаюсь с условиями обработки и использования моих персональных данных")).shouldBe(visible);
    }
    ...
    void invalidCity() { // не валидный город
        ...
        $("[data-test-id='city'] input").shouldBe(visible, enabled).setValue("Китай");
        $("[data-test-id='date'] input").sendKeys(Keys.chord(Keys.SHIFT, Keys.HOME), Keys.BACK_SPACE, planingDate);
        $("[data-test-id='name'] input").setValue("Иванов Иван Иванович");
        $("[data-test-id='phone'] input").setValue("+79999999999");
        $("[data-test-id='agreement']").click();
    ...
    @Test
    void invalidDate() { // не верная дата 27.57.6001"
        Selenide.open("http://localhost:9999/");

        $("[data-test-id='city'] input").shouldBe(visible, enabled).setValue("Ниж");
        $$(".menu-item").findBy(text("Нижний Новгород")).shouldBe(visible).click();
        $("[data-test-id='date'] input").sendKeys(Keys.chord(Keys.SHIFT, Keys.HOME), Keys.BACK_SPACE);
        $("[data-test-id='date'] input").setValue("27.57.6001");
        $("[data-test-id='name'] input").setValue("Иванов Иван Иванович");
        $("[data-test-id='phone'] input").setValue("+79999999999");
        $("[data-test-id='agreement']").click();
        $$("button").findBy(text("Забронировать")).click();
    ...
    @Test
    void repeatedInvalidDate() { // не верная дата 28.05.2025
        Selenide.open("http://localhost:9999/");

        $("[data-test-id='city'] input").shouldBe(visible, enabled).setValue("Ниж");
        $$(".menu-item").findBy(text("Нижний Новгород")).shouldBe(visible).click();
        $("[data-test-id='date'] input").sendKeys(Keys.chord(Keys.SHIFT, Keys.HOME), Keys.BACK_SPACE);
        $("[data-test-id='date'] input").setValue("28.05.2025");
        $("[data-test-id='name'] input").setValue("Иванов Иван Иванович");
        $("[data-test-id='phone'] input").setValue("+79999999999");
        $("[data-test-id='agreement']").click();
        $$("button").findBy(text("Забронировать")).click();
